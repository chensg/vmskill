...
**横版那两种还可以出双语音轨**（`LANGS = ["zh", "en"]`）：一条画面 + 中英两条音轨 + 两份 SRT，传 YouTube 直接认。时间轴归中文，英文用 `langfit` 逐句压进中文定的槽。
...
- **多音轨 concat 默认只留一条轨**。不给 `-map` 时 ffmpeg 每种类型只挑一条流，
  两条音轨的片子拼完只剩中文轨 —— 而且返回码 0、能播、时长对
...
- `make_story_h.py langfit` — 双语对轴：逐句把英文压进中文定的槽，压不进去的报出还得砍几个词
