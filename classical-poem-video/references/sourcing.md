...
先量 `mean / p50 / p90 / p99 / max` 再定，做法见 `style.md`「调色」那一节。
...
## 出图任务书：承重的构图要求必须写成硬约束

《经度》段一 13 张里有 2 张没达标，两次都是同一个毛病 —— **我把要求写成了气氛描述**：

- 镜 9 要「左半边正午、右半边黄昏」（横移走过"此刻你这里是正午，而伦敦已经下午三点"），
  拿回来是一整片连续的落日港湾。**而那是全段的知识核心**，画面撑不住，旁白只能单干。
- 镜 11 要「整个画面明显倾斜（船在横摇），钟摆被甩到一侧」，拿回来是一间水平的舱室
  加一台垂直的钟。「钟在船上会失准」这个唯一的任务在画面上看不见。

段二把所有**「这一镜的意思全靠某个构图特征」**的都改成粗体硬约束：

  「两个摆锤要各自靠近画面左右两侧，中间的连杆要清楚」
  「端端正正立在画面中央，垂直，一丝不歪」
  「黄铜与钢的冷暖差别必须明显，不能都画成一个色」
  「至少两枚木齿轮的咬合处要清晰、占足够大的面积」

**13 张全部达标。**

判断哪些要写成硬约束，有个简单的问法：**这一镜的意思，是不是全靠某个具体的
构图特征？** 是的话就写死，而且写清"必须"、"不能"，不要写"最好"、"氛围上"。
气氛可以描述，承重的结构不行。

### 禁止「任何文字与字母」会把钟的指针和刻度一起干掉

段一在禁止项里写了"任何文字与字母"，成片里两台钟的表盘是**纯白空盘**，
看着像没做完。生成器把指针和数字也当成文字去掉了。

要禁的是**可读的词句**，不是刻度。段二加了这一句就正常了：

```
Clock dials must have hands and hour markings. Do not leave dials blank.
```

同理，海图上的测深数字、演算纸上的算式都属于"该有的数字"，
禁止项要写成「任何**可读的词句**」而不是「任何文字与字母」。

## 写出图任务书之前，先问清生成器的分辨率天花板

**`budget` 算出来的是"想要多大"，生成器能给多大是另一回事，而后者决定分镜。**

《4321》上踩的：任务书按 `budget` 的分档写了「统一 4K」，样图回来是 **1536×2752**
（Gemini 的 2K 档，用户那边调不上去）。于是图眼那三镜的 z 上限被反推死：

```
pp = 短边 / (z × 输出短边)          # 1536 / (z × 1080)
pp >= 1.20（技能判据）  ->  z <= 1.19
pp >= 1.00（可接受下限）->  z <= 1.42     <- 实际用的
```

原来 z 最紧 1.62 的两镜（金句推近、结尾大拉远）pp 只有 0.88、细节 85%，
而它们偏偏是全片最值钱的两镜。**改分镜比改图便宜，但比一开始问清楚贵得多。**

**How to apply：**

1. **`budget` 之后、写任务书之前，先问一句生成器最大能出多少。**
   问不到就先出一张样图看尺寸——反正样图本来就要出。
2. 天花板已知时，**z 上限就是一个可以直接算的数**：`z <= 短边 / (1.00 × 输出短边)`。
   把超标的镜列出来一次改完，再跑 `check` + `budget` 复核。
3. **pp 1.00 是运镜镜的实际下限**（细节 92%），不是 1.20。1.20 是余量，
   1.00 是"还能看"。低于 1.00 就该换方案，不是硬撑。
4. 天花板要写进脚本的 `SRC_NATIVE`，**否则下一支照抄这份配置时不知道为什么 z 这么小**。
