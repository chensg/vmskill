...
def out_dir():
    """交付物（成片 / SRT / 封面 / 素材来源表）落在哪 —— **跟着素材在哪走**。
      ...
      标准布局    项目/素材  +  项目/build/脚本  -> SRC 指向上一级，交付物也往上一级写
      平铺布局    项目/素材  +  项目/脚本        -> SRC 就在手边，交付物**必须写在项目里**

    原来竖版一律写 ".."（横版 2026-09-19 就改了，竖版没跟上）。脚本放在项目根时
    ".." 就是片库根 ani/：成片、SRT、素材来源.md 全落进所有片子共用的目录，
    和上一支撞名、静默覆盖（2026-09-10《坎培拉》）。
    """
...
def check_claims():
    ...
    if hyp:
        ...
        # 「没有定论」「只是一个解释」和「证明不了」同类，原表也漏了 ——
        # 科普/荐书片自拆时用的是这两句，历史片才用「是主流推断」。
        # 两条都足够具体，不会把普通叙述误判成自拆。
        # 「一般认为」是中文里最常用的学术存疑式，原表也漏了 —— 讲考古/出土文献的
        # 片子自拆时用的正是它（《娘，再寄点钱来》：「那座墓的主人，一般认为就是衷」）。
        # 同样足够具体：普通叙述不会用「一般认为」，不会把正常句子误判成自拆。
        # ⚠️ 繁简两套都要列。稿子是繁體时（台湾观众那条线，audience-profile），
        # 只列简体等于这个检查**永远不会报警** —— 2026-09-16《他說他看反了羅盤》
        # 踩到：片里明明有自拆句，check 照样报「没有自我拆解」。
        # 这是 checks-that-never-fire 的又一例：判据的字符集要和素材的字符集对齐。
...
def selftest_credits():
    ...
    **别的**告警（实际撞上的是 `MUSIC_MODE='library'` 还没填 `MUSIC_FROM_LIBRARY`）：
    ...
    IMG_SOURCE = "generated"
    CREDITS = {CLIPS[0]["src"]: dict(title="T", holder="H", source="S",
                                     license="CC BY 4.0", url="U")}
    txt = "\n".join(credits(dry=True))
    c_ok = (CLIPS[0]["src"] in txt and "CC BY 4.0" in txt
...
def prep_key(i, c):
    """imgNN 由哪些输入做出来。任何一项变了，旧产物作废。"""
    return _key("prep", i, c, GRADE, PREP, _sig(os.path.join(SRC, c["src"])))
...
def credits(dry=False):
    ...
    if IMG_SOURCE == "found":
        ...
        if not reg:
            ...
            lines += ["", "| 文件 | 作品 | 收藏/权利人 | 来源 | 授权 | 链接 |",
                      "|---|---|---|---|---|---|"]
            for k in reg:
                e = CREDITS[k]
                lines.append("| %s | %s | %s | %s | %s | %s |"
                             % (k, e.get("title", "**缺**"), e.get("holder", "**缺**"),
                                e.get("source", "**缺**"), e.get("license", "**缺**"),
                                e.get("url", "**缺**")))
    lines += ["", "## 配乐", ""]
    if not music_on():
...
def split_tracks(multi):
    ...
    **为什么不能直接传多音轨文件**：YouTube 只认上传件里的第一条/默认音轨，
    ...
    """
    base = out_base()
    upload = os.path.join(out_dir(), OUT_NAME)
    ...
    if len(LANGS) == 1:
        ...
        if SUB_MODE != "burn":
            print("字幕: %s（外挂，跟着成片一起发）"
                  % os.path.join(out_dir(), srt_name(DEFAULT_LANG)))
        return

    # ---- 多音轨：先渲一份全都在的，再拆成能传的 ----
    multi = os.path.join(out_dir(), "%s_多音轨.mp4" % out_base())
    run(["ffmpeg", "-y", "-v", "error", "-stats"] + ins
        + ["-filter_complex", ";".join(fc), "-map", "[v]"] + amaps
